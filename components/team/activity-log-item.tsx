  ...
  return (
    ...
    >
      ...
      <div className="bg-surface-container-low rounded-xl p-4 hover:border-outline hover:shadow-sm transition-all space-y-3.5 cursor-pointer">
        ...
        {/* Linked GitHub commits and PRs */}
        {log.githubLinks && log.githubLinks.length > 0 && (
          <div className="space-y-1 pt-0.5">
            <span className="text-[8px] font-extrabold uppercase text-outline tracking-wider block">GitHub Activities</span>
            <div className="flex flex-wrap gap-1">
              {log.githubLinks.map((link: any, idx: number) => (
                <a
                  key={idx}
                  href={link.url}
                  target="_blank"
                  ...
                  className="flex items-center gap-1 px-2 py-0.5 rounded border border-outline-variant bg-surface-container/60 hover:bg-primary/10 hover:border-primary/30 text-[9px] font-semibold text-on-surface-variant hover:text-primary transition-all"
                  title={`${link.repo_name}: ${link.title}`}
                >
                  <Github className="h-3 w-3 shrink-0" />
                  <span>
                    {link.link_type === "commit"
                      ? `[Commit] ${link.entity_id.slice(0, 7)}`
                      : `[PR] #${link.entity_id}`}
                  </span>
                </a>
              ))}
